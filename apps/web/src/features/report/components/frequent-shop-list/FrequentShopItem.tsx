  ...
  const content = (
    <>
      ...
      <span className="flex min-w-0 items-center justify-between gap-2">
        ...
        <span className="flex shrink-0 items-end gap-1 px-1 text-neutral-700">
          <span className="sr-only">방문 횟수</span>
          <strong className="text-heading-03-semibold tabular-nums" data-mp-mask="">
            {visibleVisitCount}
          </strong>
  ...
  return (
    <li className="px-4">
      {onSelect ? (
        <button
          type="button"
          aria-labelledby={shopActionLabelId}
          onClick={onSelect}
        ...
        >
          <span id={shopActionLabelId} className="sr-only">
            <span data-mp-mask="">{visiblePlaceName}</span> 지도에서 보기
          </span>
          {content}
        </button>
      ) : (
