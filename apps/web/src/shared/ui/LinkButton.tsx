 ...
 * 기본적으로 전체 너비와 `xlarge` 크기(54px)로 렌더링됩니다.
 * `className`으로 기본 스타일을 재정의할 수 있습니다.
...
export function LinkButton(props: LinkButtonProps) {
  const { className, size = 'xlarge', variant = 'primary', ...restProps } = props;

  return <Link className={cn(buttonVariants({ size, variant }), className)} {...restProps} />;
}
